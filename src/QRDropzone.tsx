...
const QRDropzone: React.FC = () => {
  ...
  const exportExcel = () => {
    const dataForExport = groupedData.map(item => ({
      model_name: item.model_name,
      lot: item.lot,
      'จำนวนรวม (ในกล่อง)': item.total_quantity,
      Box: item.files.length,
    }));
          ...
          <table style={{ width: '100%', fontSize: 16, borderCollapse: 'collapse', minWidth: 500 }}>
            <thead style={{ background: '#e3f2fd' }}>
              <tr>
                ...
                <th style={{ padding: 12, borderBottom: '1px solid #bbdefb', color: '#1976d2', fontWeight: 700 }}>จำนวนรวม (ในกล่อง)</th>
                <th style={{ padding: 12, borderBottom: '1px solid #bbdefb', color: '#1976d2', fontWeight: 700 }}>Box</th>
              </tr>
            </thead>
            <tbody>
              {groupedData.length === 0 ? (
                ...
                <tr key={d.model_name + d.lot} style={{ background: i % 2 ? '#f5fafd' : undefined }}>
                  ...
                  <td style={{ padding: 12, borderBottom: '1px solid #bbdefb', textAlign: 'center' }}>{d.total_quantity}</td>
                  <td style={{ padding: 12, borderBottom: '1px solid #bbdefb', textAlign: 'center' }}>{d.files.length}</td>
                </tr>
              ))}
            </tbody>
          </table>
